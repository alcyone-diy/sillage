...
final class DefaultBackgroundMonitoringServiceTests: XCTestCase {
  ...
  func testStartMonitoring_requestsTokens() async throws {
    let mockPositioning = MockPositioningService()
    let service = DefaultBackgroundMonitoringService(positioningService: mockPositioning)

    // Act
    let token = service.startMonitoring(
      ownerIdentifier: "test",
    ...
    try await waitFor {
      mockPositioning.lastBackgroundToken?.invalidateCallCount == 1 &&
      mockPositioning.lastUpdateToken?.invalidateCallCount == 1
    }
