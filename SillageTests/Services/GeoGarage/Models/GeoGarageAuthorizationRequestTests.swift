...
final class GeoGarageAuthorizationRequestTests: XCTestCase {
  ...
  func testURLCarriesEveryPKCEParameter() throws {
    ...
    // Foundation.URL.path retire toujours le "/" final (constaté aussi sur l'URL de l'endpoint
    // avant toute reconstruction) : "/o/authorize/" et "/o/authorize" désignent la même ressource,
    // absoluteString ci-dessous vérifie que le "/" d'origine est bien conservé dans l'URL produite.
