    ...
    for i in 0..<activeDownloads.count {
      ...
      if case .queued = activeDownloads[i].phase {
        ...
        downloadTasks[targetID] = Task { @MainActor [weak self] in
          ...
          // Voie B (11 sept. 2026) : plus de clé CaaS embarquée, le paquet est demandé avec le
          // jeton de l'utilisateur connecté. Le trousseau fait foi (le refresh de la Task 5a.2 y
          // range un jeton neuf en cours de session) ; le jeton mis en cache par startDownload ne
          // sert que de secours, notamment quand le trousseau est vide sous test.
